<?php
    ...
    class OrderReport {
        ...
        public static function getDayAverageSale($date) {
            ...
            $stmt = $mysqli->prepare(
                "SELECT AVG(order_items.quantity * products.price + orders.tax + orders.shipping_fee - orders.discount) as 'average_sale'
                FROM order_items 
                LEFT JOIN products ON products.id = order_items.product_id 
                INNER JOIN orders ON orders.id = order_items.order_id
                WHERE DATE(orders.created_at)=? AND 
        ...
        public static function getWeekAverageSale($year, $week) {
            ...
            $stmt = $mysqli->prepare(
                ...
                LEFT JOIN products ON products.id = order_items.product_id 
                INNER JOIN orders ON orders.id = order_items.order_id
                WHERE YEAR(orders.created_at)=? AND 
                WEEK(orders.created_at, 0)=? AND 
                payment_status = 'Completed'"
            );
        ...
        public static function getDayRevenue($date) {
            ...
            $stmt = $mysqli->prepare(
                "SELECT sum(order_items.quantity * products.price) + orders.tax + orders.shipping_fee - orders.discount as revenue 
                FROM order_items 
                LEFT JOIN products ON products.id = order_items.product_id 
                INNER JOIN orders ON orders.id = order_items.order_id
        ...
        public static function getWeekRevenue($year, $week) {
            ...
            $stmt = $mysqli->prepare(
                "SELECT sum(order_items.quantity * products.price) + orders.discount - (orders.tax + orders.shipping_fee) as revenue 
                FROM order_items 
                LEFT JOIN products ON products.id = order_items.product_id 
                INNER JOIN orders ON orders.id = order_items.order_id
                WHERE YEAR(orders.created_at)=? AND
        ...
        public static function getMonthRevenue($year, $month) {
            ...
            $stmt = $mysqli->prepare(
                ...
                INNER JOIN orders ON orders.id = order_items.order_id
                WHERE YEAR(orders.created_at)=? AND 
                MONTH(orders.created_at)=? AND payment_status = 'Completed';"
            );
            $stmt->bind_param("ii", $year, $month);
            $stmt->execute();
            $stmt->bind_result($revenue);
